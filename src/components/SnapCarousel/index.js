...
export default class SnapCarousel extends Component {
    ...
    render() {
        const defaultLayout = this.mainExample(1, 'Default layout | Loop | Autoplay | Parallax | Scale | Opacity | Pagination with tappable dots');
        ...
        const { slider1ActiveSlide } = this.state;
        const { layout, data, autoplay, autoplayDelay, autoplayInterval, customItemWidth, customSliderWidth } = this.props;
        ...
        return (
            <View>
                <Carousel
                    ...
                    renderItem={this._renderItemWithParallax}
                    sliderWidth={sliderWidth}
                    itemWidth={customItemWidth ? customItemWidth : itemWidth}
                    hasParallaxImages={true}
                    firstItem={SLIDER_1_FIRST_ITEM}
                    inactiveSlideScale={0.94}
                    inactiveSlideOpacity={0.7}
                    ...
                    autoplayDelay={autoplayDelay ? autoplayDelay : 500}
                    autoplayInterval={autoplayInterval ? autoplayInterval : 3000}
                    onSnapToItem={(index) => this.setState({ slider1ActiveSlide: index })}
                />
                <Pagination
                    ...
                    activeDotIndex={slider1ActiveSlide}
                    containerStyle={styles.paginationContainer}
                    dotColor='rgba(52, 152, 219,1.0)'
                    dotStyle={styles.paginationDot}
                    inactiveDotColor='rgba(52, 152, 219,0.5)'
                    inactiveDotOpacity={0.4}
                    inactiveDotScale={0.6}
                    carouselRef={this._slider1Ref}
                    tappableDots={!!this._slider1Ref}
